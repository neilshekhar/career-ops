...
import { copyFileSync, readFileSync, writeFileSync, existsSync, unlinkSync, rmSync, mkdirSync, realpathSync } from 'fs';
...
const SYSTEM_PATHS = [
  ...
  'match-star.mjs',
  'jd-skill-gap.mjs',
  // Fail-closed tombstone: shipping it overwrites unsafe legacy copies on update.
...
// A preserved custom HTML/CSS file may still reference an asset retired
// upstream. Keep that asset so preserving the file also preserves its output.
...
async function check() {
  ...
  const remoteRef = await curlGet('https://api.github.com/repos/neilshekhar/career-ops/git/ref/heads/main', [
...
async function apply() {
  ...
  try {
    ...
    // exclusions so the index never briefly contains the upstream blob.
