...
- **Оценивает вакансии** по структурированной системе A–G (10 взвешенных параметров оценки и анализ легитимности)
...
| Функция                       | Описание                                                                                                                                                  |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Авто-пайплайн**             | Вставьте URL: сначала оценка/балл и вердикт; персонализированные материалы и работа с заявкой требуют явного продолжения или выбора в дашборде             |
| **Оценка A–G**                | Резюме роли, совпадение с CV, стратегия грейда, исследование компенсации, персонализация, подготовка к интервью (STAR+R) и проверка легитимности (Блок G) |
...
| **Локальный kanban-дашборд**  | Дашборд для ревью и подготовки ролей, постановки запросов на заявку в очередь активного агента и проверки готовой к ревью работы                           |
...
npx playwright install chromium   # нужно для PDF и браузерной проверки актуальности через Playwright
...
/career-ops {вставь описание JD}  → Сначала оценка/балл и вердикт; ждать явного продолжения или выбора в дашборде
...
Или просто вставьте URL вакансии или описание — career-ops автоматически распознает его, оценит и сначала покажет вердикт. Согласно `modes/_custom.md`, система не создаёт персонализированные материалы, не переводит статус заявки в трекере дальше, не выбирает роль, не открывает браузер и не заполняет живую форму до вашего явного продолжения или выбора роли в дашборде.
...
└────────┬─────────┘
         │
┌────────┼─────────┐
│  Оценка A–G      │  Совпадение, пробелы, компенсация, STAR-истории, легитимность
│  (читает cv.md)  │
└────────┬─────────┘
         │
         ▼
Балл/вердикт + отчёт + статус трекера: Evaluated
         │
         ▼
Явное продолжение или выбор в дашборде
         │
         ▼
Персонализированные материалы → браузер/заполнение → проверка и отправка кандидатом
```
...
## Локальный kanban-дашборд

Локальный kanban-дашборд — основной интерфейс ревью в career-ops. Он работает только на вашей машине, открывается в браузере и раскладывает роли по Inbox, To Do, Prepared, In Review и Done.

```bash
npm run launch   # открыть http://127.0.0.1:7777
```

Используйте его, чтобы смотреть scores, выбирать роли для подготовки, ставить в очередь долговечные запросы на заявку для активного агента, проверять черновики/файлы и сохранять ручное подтверждение перед отправкой. Дашборд сам не запускает автоматизацию браузера и не заполняет или отправляет заявки.

### Terminal Tracker TUI

Терминальный дашборд остаётся дополнительным видом для tracker/отчётов:
...
├── dashboard/                   # Локальный kanban-дашборд + Go TUI tracker
...
Нет. career-ops — это фильтр, а не автоматический рассыльщик. Для явно выбранных
вакансий он может подготовить материалы и заполнить форму для вашей проверки, но
никогда не нажимает финальную кнопку отправки. Окончательный выбор всегда за вами.
