...
سيحمّل الوكيل أغلفة اللغة من هذا المجلد **مع** مسارات العمل الأساسية في `modes/`؛ ملفات اللغة لا تستبدل قواعد التنفيذ الأساسية.
...
| `fursah.md` | `modes/oferta.md` (EN) | غلاف لغوي عربي؛ ينفذ التقييم الأساسي A-G دون إعادة تعريفه |
| `takdeem.md` | `modes/apply.md` (EN) | غلاف لغوي عربي ينفذ مسار العمل الأساسي دون تغييره |
| `pipeline.md` | `modes/pipeline.md` (EN) | غلاف لغوي عربي؛ ينفذ مسار pipeline الأساسي دون إعادة تعريفه |
...
- حالات الطلبات في جدول التتبع (`Evaluated` و `Applied` و `Responded` و `Interview` و `Offer` و `Hired` و `Rejected` و `Discarded` و `SKIP`).
...
4. عدّل اللغة والمصطلحات الإقليمية فقط؛ لا تنسخ أو تعيد تعريف مسارات العمل الأساسية داخل ملفات اللغة.
