...
2. Only when the checker is inconclusive or the host is unsupported, inspect a Playwright
   snapshot. Reuse an existing `auto-pipeline` snapshot when available. When
   `scan.extractor: cli` is configured, `node browser-extract.mjs <url>` may provide a
   compact read-only snapshot; fall back silently to `browser_navigate` +
   `browser_snapshot` if it fails or is missing. Title + substantive JD or a genuine
   application path is active evidence. Expired/closed text, 404/410, a generic careers
   redirect, or nav/footer without a JD is closed evidence.
