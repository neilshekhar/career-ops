...
9. Rewrite Professional Summary by injecting JD keywords and the candidate's exit narrative from `modes/_profile.md`. If no personalized narrative exists, use only facts from `cv.md`; never reuse examples from this mode as candidate history.
10. Select up to 3-4 relevant projects that actually exist in the source files. Fewer is correct when fewer are supported.
...
17. Run: `node build-cv-html.mjs /tmp/cv-{candidate}-{company}.json output/cv-{candidate}-{company}-{YYYY-MM-DD}.html {template}` — where `{template}` is the path printed by **Selecting the template** below (omit the argument to use the base `cv-template.html`). The script owns the markup and escaping. Keep this dated HTML beside the same-basename PDF because the dashboard regenerates from it and `verify-userdata.mjs` verifies it.
18. Run the fact gate: `node verify-cv-facts.mjs output/cv-{candidate}-{company}-{YYYY-MM-DD}.html`
