...
<p align="center">
  <sub>🍴 Este é o <a href="https://github.com/neilshekhar/career-ops"><strong>neilshekhar/career-ops</strong></a>, mantido e estendido por <a href="https://github.com/neilshekhar">Neil Shekhar</a> — um fork do <a href="https://github.com/santifer/career-ops">career-ops</a> original criado por <a href="https://santifer.io">santifer</a>.</sub>
</p>

<p align="center">
  <em>Se candidatar a vagas do jeito difícil leva meses. Este é o sistema criado para resolver isso.</em><br>
  Empresas usam IA para filtrar candidatos. <strong>Isso coloca a IA nas mãos dos candidatos para <em>escolher</em> empresas.</strong><br>
  <em>Open source.</em>
</p>

<p align="center"><sub>★ Reconhecimento de imprensa conquistado pelo projeto original <a href="https://github.com/santifer/career-ops">career-ops</a></sub></p>
...
- **Avalia vagas** com um sistema estruturado A-G (10 dimensões de pontuação mais análise de legitimidade)
...
Originalmente criado por [santifer](https://santifer.io), que o usou para avaliar 740+ vagas, gerar 100+ CVs personalizados e conquistar uma posição de Head of Applied AI. [Leia o estudo de caso completo](https://santifer.io/career-ops-system).

## Funcionalidades

| Funcionalidade                       | Descrição                                                                                                                                      |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **Auto-Pipeline**                    | Cole uma URL: primeiro avaliação/pontuação e veredito; materiais personalizados e candidatura exigem continuação explícita ou seleção no dashboard |
| **Avaliação A-G**                    | Resumo da vaga, aderência ao CV, estratégia de senioridade, pesquisa de compensação, personalização, preparação para entrevista (STAR+R) e verificação de legitimidade (Bloco G) |
...
| **Dashboard kanban local**           | Dashboard para revisar e preparar vagas, enfileirar solicitações de candidatura para o agente ativo e conferir trabalho pronto para revisão    |
...
npx playwright install chromium   # necessário para PDFs e verificação de navegador/liveness com Playwright
claude
```
...
/career-ops {cole um JD}   → Avaliação/pontuação e veredito primeiro; aguarde continuação explícita ou seleção no dashboard
...
Ou apenas cole uma URL ou descrição de vaga diretamente -- career-ops detecta, avalia/pontua e mostra primeiro o veredito. Conforme `modes/_custom.md`, ele não gera materiais personalizados, não avança o status da candidatura no tracker, não seleciona uma vaga, não abre o navegador nem preenche o formulário ao vivo até você continuar explicitamente ou selecionar a vaga no dashboard.
...
└────────┬─────────┘
         │
┌────────▼─────────┐
│  Avaliação A-G   │  Aderência, gaps, compensação, histórias STAR, legitimidade
│  (lê cv.md)      │
└────────┬─────────┘
         │
         ▼
Pontuação/veredito + relatório + status do tracker: Evaluated
         │
         ▼
Continuação explícita ou seleção no dashboard
         │
         ▼
Materiais personalizados → navegador/preenchimento → revisão e envio pelo candidato
```
...
Use para revisar scores, escolher vagas para preparar, enfileirar solicitações duráveis de candidatura para o agente ativo, conferir rascunhos/arquivos e manter revisão humana antes do envio. O dashboard não inicia automação do navegador nem preenche ou envia candidaturas por conta própria.
...
│   ├── _shared.md               # Contexto compartilhado (camada de sistema — personalize _profile.md/_custom.md em vez disso)
...
career-ops foi **originalmente criado por [Santiago Fernández de Valderrama (santifer)](https://santifer.io)** -- Head of Applied AI e ex-fundador (construiu e vendeu uma empresa que ainda opera com seu nome). Ele construiu o career-ops para gerenciar sua própria busca de emprego, e o usou para conquistar seu cargo atual. Seu portfólio e outros projetos open source → [santifer.io](https://santifer.io).

**Este repositório, [neilshekhar/career-ops](https://github.com/neilshekhar/career-ops), é um fork mantido e estendido por [Neil Shekhar](https://github.com/neilshekhar)** -- com correções e funcionalidades adicionais construídas sobre o trabalho original do santifer, e sincronizado periodicamente com o upstream.
