...
user_invocable: true
user-invocable: true
argument-hint: "[scan | discover | deep | pdf | latex | latex-tex | cover | email | add | expand | eu-swe | oferta | ofertas | apply | batch | tracker | agent-inbox | pipeline | contacto | training | project | interview-prep | interview | interview-redflag | interview/plan | interview/practice | interview/debrief | patterns | offer-prep | titles | upskill | followup | queue | reply-watch | update]"
license: MIT
---
...
after `modes/_profile.md` and before the selected mode file for **every** mode. It holds the user's
...
`DATA_CONTRACT.md`). It never adds factual claims about the candidate. It may be absent
on a fresh setup; if so, just skip it.
...
Applies to: `tracker`, `agent-inbox`, `reply-watch`, `deep`, `interview-prep`, `interview`, `interview-redflag`, `regional/eu-swe`, `interview/plan`, `interview/practice`, `interview/debrief`, `latex`, `latex-tex`, `training`, `project`, `patterns`, `titles`, `upskill`, `followup`, `queue`, `cover`, `email`, `add`, `offer-prep`, `discover`
...
For `scan` and `pipeline` (3+ URLs), a bounded worker/subagent may receive
`_shared.md` + `_profile.md` (if exists) + `_custom.md` (if exists) +
`modes/{mode}.md`. A live `apply` run stays with the single browser controller
described above; never launch a parallel browser-backed apply worker. If your CLI
exposes an `Agent(...)` primitive, a permitted worker call looks like this:
