...
export function checkInstructionSurface(file, source, { skill = false } = {}) {
  ...
  return requirePatterns(file, source, [
    ...
    ['per-page lookup step', /--lookup/],
    ['L3 novel-answer step', /\bL3\b/],
    ['per-page teach barrier', /--teach/],
    ['empty teach barrier', /including\s+`\[\]`/i],
    ['verification before Next', /verify loop before Next/i],
    ...
    ['receipt begin/page/finalize barrier', /--begin[\s\S]*--page[\s\S]*--finalize/],
...
export function checkAutofillOverview(file, source) {
  const errors = requirePatterns(file, source, [
    ...
    ['per-page lookup', /`--lookup`/],
    ['L3 completion', /\bL3\b/],
    ['per-page teach', /`--teach`/],
    ['empty teach barrier', /including `(?:answers:)?\[\]`/i],
    ['rendered-state verification', /verify the rendered state/i],
...
export function checkCanonicalApplyMode(file, source) {
  const errors = requirePatterns(file, source, [
    ...
    ['per-page lookup', /queue-resolve\.mjs --lookup/],
    ['L3 completion', /Generate and fill every novel field now \(L3\)/],
    ['per-page teach barrier', /Teach this page before Next — mandatory barrier/],
    ['receipt begin', /application-receipt\.mjs --begin/],
    ['receipt page', /application-receipt\.mjs --page/],
    ['receipt finalizer', /application-receipt\.mjs --finalize/],
    ['stable browser control identity', /`control_id`/],
...
export function checkCustomApplyContract(file, source) {
  return requirePatterns(file, source, [
    [
      'mandatory five-file current-contract read',
      /read all five current contracts:[\s\S]{0,240}`modes\/_shared\.md`[\s\S]{0,120}`modes\/apply\.md`[\s\S]{0,120}`modes\/_custom\.md`[\s\S]{0,120}`queue-resolve\.mjs`[\s\S]{0,120}`application-receipt\.mjs`/i,
    ],
    ...
    ['executable receipt finalizer', /application-receipt\.mjs --begin[\s\S]*--page[\s\S]*--finalize/],
