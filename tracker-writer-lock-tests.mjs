...
await runWhileLocked({
  ...
  verifyOutput: stdout => stdout.includes('Written atomically under the tracker lock')
    && stdout.includes('backup: applications.md.bak'),
});
...
await runWhileLocked({
  ...
  verifyOutput: stdout => stdout.includes('Apply recommended status updates through set-status.mjs?'),
});
