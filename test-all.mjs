...
if (
  zhShared.includes('read root `modes/_shared.md`') &&
  zhShared.includes('Localization may change language and regional terminology only') &&
  zhShared.includes('It may never omit') &&
  zhOferta.includes('read and execute `modes/oferta.md`') &&
  zhOferta.includes('Retain every root gate and output requirement') &&
  zhOferta.includes('must never change')
) {
  pass('Chinese wrappers inherit canonical company-type compensation reliability checks');
} else {
  fail('Chinese wrappers do not preserve the canonical compensation contract');
}
...
// MCP fallback — so the flag actually reaches the JD paths, not just scan/pipeline.
{
  ...
  // Locale pipeline files are wrappers over the canonical root mode. They
  // inherit extractor behavior instead of copying a workflow that can drift.
  ...
  const langMissing = langPipelines.filter((m) => {
    const src = readFile(m);
    return !(src.includes('modes/pipeline.md') && /wrapper|overlay/i.test(src));
  });
  if (langPipelines.length > 0 && langMissing.length === 0) {
    pass(`all ${langPipelines.length} language pipeline wrappers inherit the canonical extractor (#1449 Phase 2b)`);
  } else {
    fail(`language pipeline wrappers missing canonical inheritance: ${langMissing.join(', ') || '(none found)'}`);
  }
}
...
try {
  ...
  const trackerWriter = readFile('tracker.mjs');
  const trackerExportIsTransactional =
    (trackerWriter.match(/await\s+openTrackerTransaction\s*\(/g) || []).length >= 1 &&
    trackerWriter.includes('trackerTransaction.read()') &&
    trackerWriter.includes('trackerTransaction.replace(out)') &&
    trackerWriter.includes('trackerTransaction?.close()');
  const trackerDeleteIsLocked =
    trackerWriter.includes('await acquireTrackerLock(') &&
    trackerWriter.includes('writeFileAtomic(MD_PATH, newContent)') &&
    trackerWriter.includes('lock?.release()');
  if (trackerExportIsTransactional && trackerDeleteIsLocked) {
    pass('tracker export and delete each hold the shared lock across read and atomic replacement');
  } else {
    fail('tracker export/delete escapes the shared tracker lock transaction');
  }

  const replyWriter = readFile('reply-watch.mjs');
  if (
    replyWriter.includes('applyConfirmedTrackerStatus') &&
    replyWriter.includes('SET_STATUS_FILE') &&
    replyWriter.includes("'--external'") &&
    !/writeFileAtomic\(\s*APPS_FILE|writeFileSync\(\s*APPS_FILE/.test(replyWriter)
  ) {
    pass('reply-watch delegates confirmed tracker changes to the canonical locked writer');
  } else {
    fail('reply-watch bypasses set-status.mjs or writes applications.md directly');
  }
  ...
  if (
    dashboardBody.includes('canonicalStatusWriter(careerOpsPath, args...)') &&
    dashboardBody.includes('"--json"') &&
    !/os\.(?:ReadFile|WriteFile)|writeFileAtomic|acquireTrackerLock/.test(dashboardBody)
  ) {
    pass('dashboard delegates tracker changes to the canonical cross-runtime writer');
  } else {
    fail('dashboard tracker update bypasses the canonical cross-runtime writer');
  }
} catch (e) {
...
const claudeInstructions = readFile('CLAUDE.md');
const claudeImportsAgents = /^@(?:\.\/)?AGENTS\.md$/m.test(claudeInstructions);
const claudeKeepsReviewedContract =
  claudeInstructions.includes('## Live Application Execution Contract') &&
  claudeInstructions.includes('## Stack and Conventions') &&
  claudeInstructions.includes('## Cross-Agent QC');
if (
  claudeImportsAgents &&
  claudeKeepsReviewedContract
) {
  pass('CLAUDE.md imports AGENTS.md and retains its reviewed CLI-specific contract');
} else {
  fail('CLAUDE.md must import AGENTS.md and retain the reviewed live-application/QC contract');
}
...
console.log('\n12d. Skill entrypoint materialization');
...
console.log('\n12d. Materialized skill index mode');
...
try {
  ...
  if (
    ...
    pass('AGENTS.md and CLAUDE.md delegate onboarding state and autoCopied to doctor --json');
  } else {
    fail('AGENTS.md/CLAUDE.md onboarding state docs do not delegate to doctor --json');
  }
} catch (e) {
...
try {
  ...
  // Probe the executable directly through the shared argv-based helper. Do not
  // pass a shell pipeline as run()'s executable: run() intentionally accepts
...
try {
  ...
  if (
    ...
    claudeMd.includes('## Source-of-Truth Boundary')
  ) {
    pass('AGENTS.md and CLAUDE.md route procedural custom rules without making them factual sources');
  } else {
    fail('AGENTS.md/CLAUDE.md custom-rule source boundary is incomplete (#1198, #1736)');
  }
