...
// lower rank is never an advance. Discarded/SKIP rows have no rank: an import
// never resurrects a row the candidate deliberately closed.
...
// Tracker additions are untrusted text. Normalize every career-ops-owned cell,
// including numeric/status/PDF/report fields, before it reaches Markdown. User-
// defined columns copied from an existing row are deliberately left byte-for-
// byte so a metadata-only update cannot rewrite user content.
...
function buildRow(o) {
  ...
  put('via', sanitizeCell(o.via) || '—');
  put('role', o.role);
  put('location', sanitizeCell(o.location) || '—');
  put('score', o.score);
  put('status', o.status);
  put('pdf', o.pdf);
  // Optional trailing URL column — the stable natural key.
  put('url', sanitizeCell(o.url));
  ...
  let notes = sanitizeCell(o.notes);
...
/**
 * Enforce the fork's tracker-addition boundary for post-application states.
 *
 * A TSV can create or refresh an evaluation row, but it is not evidence that
 * an application or later lifecycle event happened. Lifecycle imports are
 * therefore written as Evaluated first, then promoted through the protected
 * set-status writer only when explicit external/historical provenance exists.
 *
...
// Lifecycle TSVs that require explicit provenance stay pending, and provenanced
// imports are promoted only after the Evaluated row is durably written.
...
for (const file of tsvFiles) {
  ...
  if (duplicate) {
    if (addition.requestedLifecycleStatus && !IMPORT_PROVENANCE) {
      // This is also the retry shape of a provenanced import whose Evaluated
      // row landed but whose canonical promotion did not. Never consume it as
      // a routine re-evaluation and silently lose the pending lifecycle event.
      console.warn(
    ...
    if (addition.requestedLifecycleStatus) {
      // A provenanced lifecycle import is status evidence, not a fresh role
      // evaluation. Keep score/report/date/notes byte-for-byte and let the
      // canonical promotion above be its only tracker effect.
      continue;
    }
    ...
    // Preserve the fork's PDF-ready metadata seam. A same-score TSV with the
    // same report identity may only promote PDF ❌→✅; it must not reset the
    // row's lifecycle state, date, notes, or other metadata. Conversely, a
    // stale lower-score TSV cannot smuggle in a PDF flag or overwrite the newer
    // evaluation merely because it carries ✅.
    if (pdfUpgrade && newScore < oldScore) {
      console.log(
        `⏭️  Skip: ${addition.company} — ${addition.role} ` +
        `(stale PDF metadata score ${newScore} < current ${oldScore})`,
      );
      skipped++;
      continue;
    }
    if (pdfUpgrade && newScore === oldScore
      ...
      console.log(`🔄 Update: #${duplicate.num} ${addition.company} — ${addition.role} (PDF ❌→✅)`);
      ...
      if (replaceTrackerLine(duplicate.raw, updatedLine)) {
        ...
        console.error(
          ...
          `(${duplicate.company} — ${duplicate.role}) to update PDF metadata.`,
        );
    ...
    const trustedPdfUpgrade = pdfUpgrade
      && (scoreUpgrade || newScore === oldScore)
      && sameReportIdentity(addition.report, duplicate.report);
    const pdf = reportNum && pdfIndex.has(String(reportNum))
      ? '✅'
      : (reportChanged ? '❌' : (trustedPdfUpgrade ? '✅' : duplicate.pdf));
    const updatedLine = buildRow({
...
console.log(`\n📊 Summary: +${added} added, 🔄${updated} updated, ⏭️${skipped} skipped${deferred ? `, ⏸️${deferred} deferred` : ''}${failedAdditions.length ? `, ❌${failedAdditions.length} NOT merged` : ''}`);
if (DRY_RUN) console.log('(dry-run — no changes written)');
trackerLock.release();

// Promote explicit external/historical lifecycle imports only through the
// canonical protected writer, after the Evaluated row is durable and the
// shared tracker lock has been released. Keep source TSVs pending until every
// promotion succeeds so retries cannot lose a lifecycle event.
...
if (!DRY_RUN) {
  ...
  if (!promotionFailed) {
    // Move only files whose effects were durably applied. Failed updates and
    // unprovenanced lifecycle claims remain replayable in tracker-additions/.
    const archivable = tsvFiles.filter(
      (file) => !failedAdditions.includes(file) && !deferredFiles.has(file),
    );
    if (archivable.length && !existsSync(MERGED_DIR)) mkdirSync(MERGED_DIR, { recursive: true });
    for (const file of archivable) {
      renameSync(join(ADDITIONS_DIR, file), join(MERGED_DIR, file));
    }
    if (archivable.length) console.log(`\n✅ Moved ${archivable.length} TSVs to merged/`);
