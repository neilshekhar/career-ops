...
// This test isolates page-budget behavior, so provide the narrow provenance
// interface generate-pdf.mjs consumes instead of copying the full application
// provenance graph (queue, source-contract, cover-quality, and user files).
writeFileSync(join(sandbox, 'generation-provenance.mjs'), `
import { writeFileSync } from 'node:fs';

export const MIN_ONE_PAGE_UTILIZATION = 0.85;

export function printablePageBox(format = 'a4') {
  return String(format).toLowerCase() === 'letter'
    ? { format: 'letter', width_px: 720, height_px: 960 }
    : { format: 'a4', width_px: 698, height_px: 1026 };
}

export function buildPdfLayoutEvidence({ format, pageCount, measurement }) {
  const printable = printablePageBox(format);
  const height = Number(measurement?.height_px || printable.height_px);
  return {
    page_count: pageCount,
    printable,
    content: {
      top_px: Number(measurement?.top_px || 0),
      bottom_px: Number(measurement?.bottom_px || height),
      height_px: height,
    },
    printable_height_utilization: height / printable.height_px,
  };
}

export function persistPdfLayoutEvidence(pdfPath, evidence) {
  const path = pdfPath + '.layout.json';
    ...
    return {
      async newPage() {
        return {
          ...
          async evaluate() {
            return { top_px: 0, bottom_px: 1800, height_px: 1800 };
          },
          async emulateMedia() {},
