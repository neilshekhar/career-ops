...
const userLayerStart = agentLines.findIndex((line) => line.includes('**User Layer'));
const userLayerLines = [];
if (userLayerStart !== -1) {
  for (const line of agentLines.slice(userLayerStart + 1)) {
    if (line.startsWith('- ')) userLayerLines.push(line);
    else if (userLayerLines.length > 0) break;
  }
}
...
if (userLayerStart === -1) {
  ...
  // Backtick-quoted paths from the bullets immediately below the heading,
  // minus the glob suffix: `data/*` -> data/.
  const paths = [...userLayerLines.join('\n').matchAll(/`([^`]+)`/g)]
    .map(m => m[1])
