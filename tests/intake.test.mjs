...
// ── three-place registration contract (offer-prep pattern) ───────────────
{
  ...
  const userLayerBlock = agentsDoc.match(/\*\*User Layer[^\n]*\*\*:?([\s\S]*?)\n\*\*System Layer/)?.[0] ?? '';
  if (userLayerBlock.includes('`documents/*`')) {
    pass('documents/ is declared in the AGENTS.md User Layer block (feeds the gitignore guard)');
  } else {
    fail('AGENTS.md User Layer block omits `documents/*` — the gitignore regression guard skips it');
  }
}
