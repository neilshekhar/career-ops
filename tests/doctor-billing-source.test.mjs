...
const DOCTOR_TARGET = mkdtempSync(join(tmpdir(), 'co-billing-'));

/** Run doctor and return its human output (it exits non-zero when it finds issues). */
function runDoctor(env) {
  try {
    return execFileSync(NODE, [DOCTOR, '--target', DOCTOR_TARGET], {
      cwd: DOCTOR_TARGET,
      env: {
        ...process.env,
        CAREER_OPS_CLI: 'opencode',
        CLAUDE_CONFIG_DIR: DOCTOR_TARGET,
        ...env,
      },
      encoding: 'utf-8',
...
rmSync(DOCTOR_TARGET, { recursive: true, force: true });
