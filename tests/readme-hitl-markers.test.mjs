...
for (const file of readmes) {
  ...
  // Wholesale-drift control: every README describes this fork's report as A-G. This
  ...
  if (content.includes('A-G') || content.includes('A–G')) {
    pass(`${file}: mentions the A-G report structure`);
  } else {
    fail(`${file}: never mentions A-G — translation predates the current report structure`);
  }
}
