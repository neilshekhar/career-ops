...
async function resolveDnsCached(hostname) {
  ...
  // Use the mutable default export so node:test can replace DNS methods in
  // hermetic security tests. The named ESM bindings are read-only snapshots on
  // some supported Node releases, while the runtime API is otherwise identical.
