...
- **オファーを評価** -- 構造化されたA-G評価システム（10項目の重み付けスコア軸と求人正当性分析）
...
| 機能                     | 説明                                                                                                                             |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| **自動パイプライン**     | URLを貼ると、まず評価・スコアリングと判定を表示。明示的な続行指示またはダッシュボード選択後にのみ応募資料と応募作業へ進む               |
| **A-G評価**              | 役割サマリー、CVマッチ、レベル戦略、報酬調査、パーソナライズ、面接準備（STAR+R）、ブロックGの求人正当性チェック                         |
...
| **ローカルKanbanダッシュボード** | 求人のレビューと準備、アクティブなエージェントへの応募作業依頼、レビュー可能な作業の確認を行うローカルUI                 |
...
npx playwright install chromium   # PDF生成とPlaywrightによるブラウザ／掲載中確認に必要
...
/career-ops {求人票を貼る}  → まず評価・スコア・判定。明示的な続行またはダッシュボード選択を待つ
...
または、求人URLや記述を直接貼り付けるだけで、career-opsが自動検知し、評価・スコアリングして最初に判定を表示します。`modes/_custom.md` に従い、明示的に続行するかダッシュボードで求人を選択するまで、応募資料の生成、トラッカー内の応募ステータス更新、求人の選択、ブラウザの起動、ライブフォームの入力は行いません。
...
└────────┬─────────┘
         │
┌────────▼─────────┐
│  A-G 評価        │  マッチ度、ギャップ、報酬調査、STARストーリー、正当性
│  (cv.mdを読む)   │
└────────┬─────────┘
         │
         ▼
スコア/判定 + レポート + トラッカー状態: Evaluated
         │
         ▼
明示的な続行またはダッシュボード選択
         │
         ▼
応募資料 → ブラウザ/ライブ入力 → 候補者がレビューして送信
```
...
スコア確認、準備する求人の選択、アクティブなエージェントへの永続的な応募作業依頼、下書き/ファイル確認、送信前の人間によるレビューに使います。ダッシュボード自体はブラウザ自動化を起動せず、応募フォームの入力や送信も行いません。
...
├── modes/                       # 14個のスキルモード
│   ├── _shared.md               # 共有コンテキスト（システム層 — 代わりに _profile.md/_custom.md をカスタマイズ）
│   ├── oferta.md                # 単一オファー評価
│   ├── pdf.md                   # PDF生成
│   ├── scan.md                  # ポータルスキャナー
│   ├── batch.md                 # バッチ処理
