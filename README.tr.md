...
- **İlanları değerlendirir** -- yapılandırılmış A-G değerlendirmesiyle (bütüncül 1.0-5.0 puana giden beş puanlama boyutu)
...
| Özellik                  | Açıklama                                                                                                                                 |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **Otomatik Hat (Auto-Pipeline)** | Bir URL yapıştırınca önce değerlendirme ve karar görünür; PDF, özelleştirilmiş belgeler veya başvuru işi açık Continue/Dashboard seçiminizi bekler. |
...
| **Local Kanban Dashboard** | `npm run launch` ile açılan localhost-only birincil inceleme arayüzü; Terminal TUI isteğe bağlıdır. |
...
/career-ops {bir iş ilanı yapıştırın}   → Önce değerlendirme + karar; PDF/başvuru işi için açık devam seçimi gerekir
...
└────────┬─────────┘
         │
┌────────▼─────────┐
│  A-G             │  Uyum, eksikler, ücret araştırması, STAR hikayeleri
│  Değerlendirme   │
│  (cv.md okur)    │
└────────┬─────────┘
         │
...
Birincil inceleme arayüzü yalnızca yerel makinenizde çalışan Kanban panosudur:
...
career-ops kullanarak işe mi girdiniz? [Hikayenizi paylaşın!](https://github.com/neilshekhar/career-ops/issues/new?template=i-got-hired.yml)
