...
<p align="center">
  <sub>🍴 This is <a href="https://github.com/neilshekhar/career-ops"><strong>neilshekhar/career-ops</strong></a>, maintained &amp; extended by <a href="https://github.com/neilshekhar">Neil Shekhar</a> — a fork of the original <a href="https://github.com/santifer/career-ops">career-ops</a> created by <a href="https://santifer.io">santifer</a>.</sub>
</p>

<p align="center">
  <em>Applying to jobs the hard way takes months. This is the system built to fix that.</em><br>
  Companies use AI to filter candidates. <strong>This puts AI in candidates' hands to <em>choose</em> companies.</strong><br>
  <em>Open source.</em>
</p>

<p align="center"><sub>★ Press &amp; recognition earned by the original <a href="https://github.com/santifer/career-ops">career-ops</a> project</sub></p>
...
Originally built by [santifer](https://santifer.io), who used it to evaluate 740+ job offers, generate 100+ tailored CVs, and land a Head of Applied AI role. [Read the full case study](https://santifer.io/career-ops-system).
...
| **A-G Evaluation**       | Role summary, CV match, level strategy, comp research, personalization, interview prep (STAR+R), Block G posting-legitimacy checks, and a Work-Auth signal for explicit no-sponsorship blockers |
...
```bash
npm i -g @neilshekhar/career-ops
```

This installs the `career-ops` binary globally so you can run it directly instead of via `npx`. Unlike `npx @neilshekhar/career-ops init` (which bootstraps a project directory), the global install gives you a persistent `career-ops` command available anywhere in your terminal.

**Which one should you use?**
- `npx @neilshekhar/career-ops init` — best for first use; creates a dedicated project folder.
- `npm i -g @neilshekhar/career-ops` — best once you have a project folder and want to run career-ops commands directly.
...
See [docs/SETUP.md](docs/SETUP.md) for the full setup guide, [docs/MODEL_SELECTION.md](docs/MODEL_SELECTION.md) for a provider-neutral quality/cost policy, [docs/RUNNING_ON_A_BUDGET.md](docs/RUNNING_ON_A_BUDGET.md) for custom or local models, [docs/AUTOMATION.md](docs/AUTOMATION.md) for recurring scans and zero-token triage, [docs/APPLY_AUTOFILL.md](docs/APPLY_AUTOFILL.md) for the ATS auto-fill flow, and [docs/FAQ.md](docs/FAQ.md) for common questions.
...
Because an `ANTHROPIC_API_KEY` in your environment takes precedence over your logged-in subscription: the CLI uses the key and bills per token. If you configured that variable, remove it from your shell profile without printing its value, restart the terminal, and run `/login`. Batch mode is the exception, since `claude -p` workers do not use the interactive login: use the CLI's documented setup-token flow and keep the resulting token private. Full walkthrough in [docs/RUNNING_ON_A_BUDGET.md](docs/RUNNING_ON_A_BUDGET.md#2b-already-paying-for-a-subscription-make-sure-you-are-using-it).
...
No. career-ops is a filter, not a spray-and-pray auto-applier. For roles you
explicitly select, it can prepare assets and fill a live form for your review, but
it never clicks the final submission control. You always make the final call.
...
career-ops was **originally created by [Santiago Fernández de Valderrama (santifer)](https://santifer.io)** -- Head of Applied AI and former founder (built and sold a business that still runs with his name on it). He built career-ops to manage his own job search, and used it to land his current role. His portfolio and other open source projects → [santifer.io](https://santifer.io).
...
**This repository, [neilshekhar/career-ops](https://github.com/neilshekhar/career-ops), is a fork maintained & extended by [Neil Shekhar](https://github.com/neilshekhar)** -- with additional fixes and features layered on top of santifer's original work, and periodically kept in sync with upstream.
...
Got hired using career-ops? [Share your story!](https://github.com/neilshekhar/career-ops/issues/new?template=i-got-hired.yml)
...
**This fork** — [![GitHub](https://img.shields.io/badge/GitHub-neilshekhar-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/neilshekhar/career-ops)
...
**Original author (santifer)** — [![Website](https://img.shields.io/badge/santifer.io-000?style=for-the-badge&logo=safari&logoColor=white)](https://santifer.io)
