...
- **Оцінює вакансії** за структурованою системою A-G (10 зважених параметрів оцінки та аналіз легітимності)
...
| Функція                       | Опис                                                                                                                                             |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Авто-конвеєр**              | Вставте URL: спочатку оцінка/бал і вердикт; персоналізовані матеріали та робота із заявкою потребують явного продовження або вибору в дашборді |
| **Оцінка A-G**                | Опис ролі, відповідність резюме, стратегія рівня, дослідження компенсації, персоналізація, підготовка до інтерв'ю (STAR+R) і перевірка легітимності (Блок G) |
...
| **Локальний kanban-дашборд**  | Дошка для перегляду й підготовки ролей, постановки запитів на заявку в чергу активного агента та перевірки готової до перегляду роботи          |
...
git clone https://github.com/neilshekhar/career-ops.git
cd career-ops && npm install
npx playwright install chromium   # для PDF та перевірки вакансій/браузера через Playwright
claude
```
...
/career-ops {вставити JD}  → Спочатку оцінка/бал і вердикт; чекати явного продовження або вибору в дашборді
...
Або просто вставте URL вакансії чи її опис — career-ops автоматично визначить, оцінить і спочатку покаже вердикт. Згідно з `modes/_custom.md`, система не створює персоналізовані матеріали, не просуває статус заявки в трекері, не вибирає роль, не відкриває браузер і не заповнює живу форму до вашого явного продовження або вибору ролі в дашборді.
...
└────────┬─────────┘
         │
┌────────▼─────────┐
│  Оцінка A-G      │  Відповідність, прогалини, компенсація, STAR-історії, легітимність
│  (читає cv.md)   │
└────────┬─────────┘
         │
         ▼
Бал/вердикт + звіт + статус трекера: Evaluated
         │
         ▼
Явне продовження або вибір у дашборді
         │
         ▼
Персоналізовані матеріали → браузер/заповнення → перевірка й подання кандидатом
```
...
## Локальний kanban-дашборд

Локальний kanban-дашборд — основний інтерфейс перегляду в career-ops. Він працює тільки на вашій машині, відкривається у браузері та розкладає ролі по Inbox, To Do, Prepared, In Review і Done.

```bash
npm run launch   # відкриває http://127.0.0.1:7777
```

Використовуйте його для перегляду score, вибору ролей для підготовки, постановки довговічних запитів на заявку в чергу для активного агента, перевірки чернеток/файлів і ручного підтвердження перед відправкою. Дашборд сам не запускає автоматизацію браузера та не заповнює або надсилає заявки.

### Terminal Tracker TUI

Термінальний дашборд залишається додатковим переглядом для tracker/звітів:
...
│   ├── _shared.md               # Спільний контекст (системний рівень — натомість налаштуйте _profile.md/_custom.md)
...
├── dashboard/                   # Локальний kanban-дашборд + Go TUI tracker
