...
> Fork note: this repository is [Neil Shekhar's](https://github.com/neilshekhar) maintained fork of the original [santifer/career-ops](https://github.com/santifer/career-ops). Open fork-specific bugs, docs fixes, packaging changes, and release-line improvements here; upstream roadmap references below are intentionally credited to santifer's original project.
...
- **A real merged PR, on something people use.** This fork builds on a widely used original project and ships Neil's maintained release line. Your name lands in the history of a real tool, not a toy repo.
- **We answer fast.** Open an issue or PR and you'll hear back, usually within a day or two. No black holes.
- **Tiny on-ramps.** Browse [`good first issue`](https://github.com/neilshekhar/career-ops/contribute) -- each is scoped small, with a time estimate, the pattern to copy, and a clear "done", so your first PR is a win, not a maze.
...
- Report bugs via [Issues](https://github.com/neilshekhar/career-ops/issues)
...
career-ops core is **local-first and human-in-the-loop** by design — it runs on your machine and drafts applications for *you* to review and submit. Centralized infrastructure — hosted job aggregation, a shared matching service, proxies or Workers the project would operate — is **not part of the core**: it's heavier than a free local tool should carry, and it's where the original project is headed as a *separate, opt-in service*. See the upstream direction here: **[Where career-ops is going](https://github.com/santifer/career-ops/discussions/904)**.
...
- **PRs that let an agent, script, dashboard, or plugin perform final job-application submission.** career-ops may prepare and fill selected applications for review, but the candidate alone clicks the final submit control.
...
- **PRs that add centralized or hosted infrastructure to the core** (proxies, aggregation services, shared Workers). That's the separate opt-in service, not the open-core — bring it to the upstream [direction discussion](https://github.com/santifer/career-ops/discussions/904) first.
...
- [Open an issue](https://github.com/neilshekhar/career-ops/issues)
