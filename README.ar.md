...
| الميزة | الوصف |
| :--- | :--- |
| **المسار التلقائي (Auto-Pipeline)** | الصق رابط الوظيفة ليتم تقييمها ومنحها درجة وعرض القرار أولاً؛ لا تبدأ المواد المخصصة أو أعمال التقديم إلا بعد متابعة صريحة أو اختيار من لوحة التحكم |
| **تقييم A-G** | ملخص الدور، مطابقة السيرة الذاتية، استراتيجية المستوى الوظيفي، أبحاث الرواتب، التخصيص، التحضير للمقابلات (STAR+R)، وفحص شرعية الإعلان |
...
| **لوحة كانبان محلية** | لوحة متصفح محلية لمراجعة الوظائف وفرزها وتجهيزها ووضع طلبات التقديم في طابور الوكيل النشط |
| **المرشح في قلب القرار (Human-in-the-Loop)** | يقوم الذكاء الاصطناعي بالتقييم والتوصية وملء الطلبات المختارة للمراجعة، لكنه لا ينقر أبداً زر الإرسال النهائي لطلب الوظيفة؛ أنت وحدك تراجعه وتقدمه <!-- hitl: absolute guarantee. Do not add "automatically", "by itself", "without your permission" or any other hedge when translating this row. --> |
...
npx playwright install chromium   # مطلوب لملفات PDF والتحقق من المتصفح/صلاحية الإعلانات عبر Playwright
...
/career-ops {رابط الوظيفة} ← التقييم والدرجة والقرار أولاً؛ ثم انتظار متابعة صريحة أو اختيار من لوحة التحكم
...
أو يمكنك ببساطة لصق رابط الوظيفة أو وصفها مباشرة — سيتعرف career-ops عليها، ويجري التقييم ويمنح الدرجة ويعرض القرار أولاً. ووفقاً لـ `modes/_custom.md`، لن ينشئ مواد مخصصة، أو يقدّم حالة الطلب في المتتبع، أو يختار دوراً، أو يفتح المتصفح، أو يملأ نموذجاً حياً قبل أن تطلب المتابعة صراحةً أو تختار الدور من لوحة التحكم.
...
└────────┬─────────┘
         │
┌────────▼─────────┐
│  تقييم A-G       │  التوافق والفجوات والرواتب وقصص STAR وشرعية الإعلان
│ (قراءة cv.md)    │
└────────┬─────────┘
         │
         ▼
الدرجة/القرار + التقرير + حالة المتتبع: تم التقييم
         │
         ▼
متابعة صريحة أو اختيار من لوحة التحكم
         │
         ▼
مواد مخصصة → تعبئة حية في المتصفح → مراجعة المرشح والإرسال
```
...
لوحة كانبان المحلية هي واجهة المراجعة الأساسية في career-ops. تعمل على جهازك فقط، وتفتح في المتصفح، وتساعدك على نقل الوظائف بين Inbox و To Do و Prepared و In Review و Done.

```bash
npm run launch   # يفتح http://127.0.0.1:7777
```

استخدمها لمراجعة الدرجات، واختيار الوظائف التي تريد تجهيزها، ووضع طلبات تقديم دائمة في طابور الوكيل النشط، ومراجعة المسودات والملفات قبل الإرسال. لا تشغّل اللوحة أتمتة المتصفح ولا تعبّئ الطلبات أو ترسلها بنفسها.

### لوحة الطرفية الاختيارية

تبقى لوحة الطرفية متاحة كواجهة ثانوية لمراجعة التقارير والتراكر:
...
├── dashboard/                   # لوحة كانبان محلية + عارض TUI للتراكر
...
لا. career-ops أداة تصفية وليست أداة تقديم عشوائي. للوظائف التي تختارها صراحةً،
يمكنه إعداد المواد وملء النموذج لمراجعتك، لكنه لا يضغط أبداً زر الإرسال النهائي.
القرار النهائي لك دائماً.
